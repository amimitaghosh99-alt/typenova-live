...
const cardLineMatch = modalContent.match(/<div\s+className="glass-panel relative rounded-3xl bg-slate-900\/40 border border-white\/15[^"]*backdrop-blur-xl[^"]*shadow-xl[^"]*"/);
assert(cardLineMatch !== null, 'CARDS: Release cards use translucent slate glass (bg-slate-900/40, backdrop-blur-xl, border-white/15, shadow-xl)');
