      ...
      <div className="relative" ref={ref}>
        ...
        >
          ...
          <div className="p-4 border-b border-zinc-800/80 bg-zinc-950/60 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-fuchsia-500/10 to-cyan-500/10 rounded-full blur-xl pointer-events-none" />
            <p className="text-zinc-500 text-[9px] font-black tracking-widest uppercase mb-1">Ranked Elo</p>
            <div className="flex items-baseline justify-between">
              <p className={`text-3xl font-black tracking-tight ${theme.text}`}>{elo}</p>
              <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 bg-white/5 px-2 py-0.5 rounded-full border border-white/5">
                Competitive
              </span>
            </div>
          </div>
          ...
          {/* Sync Status Row */}
          <div className="px-4 py-3 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-950/30">
            <div className="flex items-center gap-2.5">
              <SyncIcon size={14} className={`${status === 'error' ? 'text-red-400' : theme.text} ${status === 'syncing' ? 'animate-spin' : ''}`} />
              <span className="text-[10px] font-black uppercase tracking-widest text-zinc-300">{STATUS_TEXT[status]}</span>
            </div>
            <span className="flex h-2 w-2 relative">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${status === 'error' ? 'bg-red-400' : 'bg-emerald-400'} opacity-75`} />
              <span className={`relative inline-flex rounded-full h-2 w-2 ${status === 'error' ? 'bg-red-500' : 'bg-emerald-500'}`} />
            </span>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3.5 text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:bg-red-500/15 hover:text-red-300 transition-all group"
          >
