    ...
    <div className={`relative w-full flex justify-center ${zenMode ? 'items-center min-h-[60vh]' : ''}`}>
      ...
      >
        <div className={`transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isCrossfading ? 'opacity-0 blur-md translate-y-8 scale-95' : 'opacity-100 blur-0 translate-y-0 scale-100'} w-full flex flex-col items-center`}>
          {/* Mode toggles — an integrated header band on the card's own
              surface (no nested panel), bled out to the card edges so it
              reads as one tile, not a second one stacked on top. */}
          {modeToggles && (
            <div className="relative z-20 w-[calc(100%+1.5rem)] md:w-[calc(100%+3rem)] -mt-1 md:-mt-3 mb-2 px-2 md:px-4 pb-2 border-b border-white/[0.07]">
              {modeToggles}
            </div>
          )}
